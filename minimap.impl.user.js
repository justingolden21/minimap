...
(async function () {
	...
	const rPlaceTemplateBaseUrl = rPlaceTemplatesGithubLfs
		? 'https://media.githubusercontent.com/media/ApionXD/minimap/d/main'
		: 'https://raw.githubusercontent.com/ApionXD/minimap/d/main';
	...
	const addRPlaceTemplate = function (templateName, options) {
		rPlaceTemplates.set(templateName, {
			...
			botUrl: options.bot
				? getRPlaceTemplateUrl(templateName, 'bot')
				: undefined,
			maskUrl: options.mask
				? getRPlaceTemplateUrl(templateName, 'mask')
				: undefined,
		});
	...
	const coordinateBlock = await new Promise((resolve) => {
		let interval = setInterval(() => {
			try {
				...
				console.error(
					'Failed to attach to coordinate block. Trying again...'
				);
			}
		}, 1000);
	});
  ...
  <audio id="noSleep" src="https://hot-potato.reddit.com/media/interactions/select-color.mp3" playsinline></audio>
	...
	class CheckboxSetting {
		...
		htmlFor(ref, id) {
			...
			return html.for(ref, id)`<div data-id=${id} class=${classes.join(
				' '
			)} onclick=${onclick}>
	...
	class Settings {
		...
		constructor(settingsBlock, mlpMinimapBlock) {
			...
			requestAnimationFrame(function measure(time) {
				render(
					settingsBlock,
					_root.htmlFor(mlpMinimapBlock, 'settings')
				);
				requestAnimationFrame(measure);
			});
		}
	...
	settings.addSetting(
		'autoColor',
		new CheckboxSetting('Auto color picker', false, function (
			autoColorSetting
		) {
	...
	settings.addSetting(
		'nosleep',
		new CheckboxSetting('NoTabFreeze (🔇 Need to mute tab)', false)
	);
	settings.addSetting(
	...
	updateTemplate = function () {
		...
		const rPlaceTemplateUrl =
			rPlaceTemplate.botUrl !== undefined &&
			settings.getSetting('bot').enabled
	...
	function findNextArt() {
		...
		for (let i = 0; i < templateData.length; i += 4) {
			...
			const isNearOtherArt = !!locations.find(
				(loc) =>
					Math.abs(x - loc.x) < NEXT_ART_MIN_DIST &&
					Math.abs(y - loc.y) < NEXT_ART_MIN_DIST
			);
		...
		if (sortedLocations.length > 0) {
			...
			console.log(
				`Moving to art at: [x: ${nextLocation.x}, y: ${nextLocation.y}]`
			);
	...
	function pickFromBuckets(buckets, position) {
		...
		const value = Array.from(buckets.keys()).reduce(
			(a, b) => Math.max(a, b),
			0
		);
	...
	function selectRandomPixelWeighted(diff) {
		...
		const position = Math.floor(
			Math.random() * Math.min(FOCUS_AREA_SIZE, totalAvailablePixels)
		);
	...
	const resizerAction = new Resizer(
		resizerBlock,
		mlpMinimapBlock,
		recalculateImagePos
	);
	...
	for (const paletteButton of paletteButtons) {
		const parsedData =
			paletteButton.children[0].style.backgroundColor.match(
	...
	(async () => {
		while (true) {
			...
			// Compute the diff
			const diffAndCisPixels = getDiff(
				botCanvas.width,
				botCanvas.height,
				botCtx,
				ctx
			...
			settings.getSetting('pixelDisplayProgress').content = html`<span
			...
			if (settings.getSetting('bot').enabled && !botLock) {
				...
				if (!embed.nextTileAvailableIn && diff.length > 0) {
					...
					embed
						...
						.then(() => {
							log(
								`Placed [x: ${randPixel.x}, y: ${randPixel.y}, color: ${selectedColor}]`
							);
						})
						.catch(() => {
							logError(
								`FAILED! [x: ${randPixel.x}, y: ${randPixel.y}, color: ${selectedColor}]`
							);
						});
